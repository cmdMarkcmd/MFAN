...
class resnet152():
    def __init__(self):
        ...
        self.model = models.resnet152().cuda()
        ...
        #对于全连接层引入合适的先验信息
        self.path = os.getcwd() + '/dataset/pheme/pheme_image/pheme_images_jpg/'
    ...
    def forward(self,x_id):
        ...
        with torch.no_grad():
            ...
            img_output =self.model(batch_img) ## Wrong
...
class TransformerBlock(nn.Module):
    ...
    def multi_head_attention(self, Q, K, V):#QKV矩阵是特征矩阵，不同的增强方式这几个输入矩阵的内容不同，基本的形状控制要在不同模块之间保持一致。
        bsz, q_len, _ = Q.size()
        bsz, k_len, _ = K.size()
        bsz, v_len, _ = V.size()
...
#所有文本相关嵌入的权重共享的多层感知机，对bert输出中存在各向异性的输出进行处理和形变相关问题的处理
    def __init__(self, input_dim=768,output_dim=300,hidden_dim_1 = 300,hidden_dim_2 = 500,attn_drop = 0.15):
        ...
        self.embedding_layer = nn.Embedding(num_embeddings = 2018,embedding_dim=input_dim
                                            ,padding_idx= 0 
                                            ,_weight = embedding_weights)
        self.embedding_layer_1 = nn.Embedding(num_embeddings = 2018,embedding_dim=input_dim
                                            ,padding_idx= 0 
                                            ,_weight = embedding_weights_1)
        self.embedding_layer_2 = nn.Embedding(num_embeddings = 2018,embedding_dim=input_dim
                                            ,padding_idx= 0 
                                            ,_weight = embedding_weights_2)
        self.embedding_layer_3 = nn.Embedding(num_embeddings = 2018,embedding_dim=input_dim
                                            ,padding_idx= 0 
                                            ,_weight = embedding_weights_3)
        self.graph_embedding = nn.Embedding(num_embeddings=4438,embedding_dim = input_dim
                                                  ,padding_idx=0
                                                  ,_weight = torch.from_numpy(embedding_weights_4))
    ...
    def forward(self,layer_id,X_id=0):
        ...
        elif X_id ==0:#不规范的调用接口，只处理两种情况
            X_ = self.graph_embedding(torch.arange(0, 4438).long().cuda()).to(torch.float32)
...
class GraphAttentionLayer(nn.Module):#单层注意力工具模块
    def __init__(self, in_features, out_features, dropout, alpha, concat=True):
        ...
        self.a = nn.Parameter(torch.zeros(size=(2 * out_features, 1))) # [(2*out_feature)*1]
        nn.init.xavier_uniform_(self.a.data, gain=1.414)
        self.wtrans = nn.Parameter(torch.zeros(size=(2 * in_features, out_features))) 
        nn.init.xavier_uniform_(self.wtrans.data, gain=1.414)
        self.leakyrelu = nn.LeakyReLU(self.alpha)

    def forward(self, inp, adj): 
        h = torch.mm(inp, self.W) # mm矩阵乘法 [n*d] * [d*out_feature]=[n*out_feature]
        N = h.size()[0]
        Wh1 = torch.mm(h, self.a[:self.out_features, :]) # [n*out_feature]*[out_feature*1]=[n*1]
        Wh2 = torch.mm(h, self.a[self.out_features:, :]) # [n*out_feature]*[out_feature*1]=[n*1]
        ...
        h_prime_double = torch.cat([h_prime,h_prime_negative],dim=1) #[n,2d]
    ...
    def fit(self,x_train_id,y_train,x_dev_id,y_dev):#整理多轮次的训练测试
        ...
        for epoch in range(config['epochs']):
            ...
            for i ,data in enumerate(dataloader):#转化成批次大小对象的迭代器了，迭代对象是dataset组
                ...
                #print("beforegetbatch")
                batch_x_id,batch_y = (item.cuda(device = self.device)for item in data)#解绑
                #print("beforegetbatch_2")
                self.batch_dealer(batch_x_id,batch_y,loss,i,total,pgd_block)
                #print('afterbatchdeal')
                if self.init_clip_max_norm is not None:
                    utils.clip_grad_norm_(self.parameters(), max_norm=self.init_clip_max_norm)
            self.batch_evaluate(x_dev_id,y_dev)

    def batch_dealer(self,x_id,y,loss,i,total,pgd_block):
        #print('inbatchone')
        self.optimizer.zero_grad()
        #print('inforward_-1')
        logit_original,dist_org = self.forward(x_id)
        #print('inbatchone_2')
        ...
        loss_defence = 3.6*loss_classify+1.8*loss_dis#设置两个超参数衡量判定损失和对齐损失
...
class EXP(NeuralNetwork):
    ...
    def forward(self,x_id):#id是张量格式
        ...
        rembedding = self.gat_relation.forward(x_id) # Signed GAT 图嵌入
        #print('inf_4')
        #[64,300]
        #[64,300]
        #[64,300]

        text_embedding_0 = self.tv_attention(text_embedding_0.view(bsz, -1, 300),iembedding.view(bsz, -1, 300)
                                                             ,iembedding.view(bsz, -1, 300))
        text_embedding_1 = self.tv_attention(text_embedding_1.view(bsz, -1, 300),iembedding.view(bsz, -1, 300)
                                                             ,iembedding.view(bsz, -1, 300))
        text_embedding_2 = self.tv_attention(text_embedding_2.view(bsz, -1, 300),iembedding.view(bsz, -1, 300)
                                                             ,iembedding.view(bsz, -1, 300))                                                   
        text_embedding_3 = self.tv_attention(text_embedding_3.view(bsz, -1, 300),iembedding.view(bsz, -1, 300)
                                                             ,iembedding.view(bsz, -1, 300))
        ...
        text_enhanced = self.relu(self.fc_tv(torch.cat( (text_embedding_0
                                                        ,text_embedding_1
                                                        ,text_embedding_2
                                                        ,text_embedding_3),dim=2)))
...
def load_dataset():
    print('data_load: Start')
    pre = os.getcwd() + "/dataset/pheme/pheme_files"
    X_train_tid, _, y_train,_, adj = pickle.load(open(pre + "/train.pkl", 'rb'))
    ...
    with open(pre+ '/new_id_dic.json', 'r') as f:
        ...
        newid2mid = dict(zip(mid2newid.values(),mid2newid.keys()))
    ...
    for i in id_list:
        num2index[i] = index
        index+=1
    config['num_toindex'] = num2index#imid->index
...
def load_original_adj(adj): # 将adj节点稀疏矩阵转化为original_adj邻接矩阵
    ...
    for i, v in original_adj_dict.items():#根据输入的adj格式建立邻接矩阵并且遍历稀疏矩阵
        v = [int(e) for e in v] 
        original_adj[int(i), v] = 1 # 可以一次性把v这个列表里面的v_n处的 original_adj[i,v_n] 设置为 1
